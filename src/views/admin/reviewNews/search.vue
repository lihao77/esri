<template>
	<div class="table-search-container" v-if="props.search.length > 0" ref="container">
		<el-form ref="tableSearchRef" :model="state.form" size="default" label-width="100px" class="table-form">
			<el-row>
				<el-col :span="colSpan" class="mb20" v-for="(val, key) in props.search" :key="key"
					...
					<template v-if="val.type !== ''">
						<el-form-item :label="val.label" :prop="val.prop"
							...
							<el-select v-model="state.form[val.prop]" :placeholder="val.placeholder"
								v-else-if="val.type === 'select'" style="width: 100%">
				...
				<el-col :span="colSpan" class="mb20">
					<el-form-item class="table-form-btn" :label-width="props.search.length <= 1 ? '10px' : '100px'">
						<template #label v-if="props.search.length > 1">
						...
						<div>
							...
							<el-button size="default" type="info" class="ml10" @click="onReset(tableSearchRef)"> 重置
							</el-button>
						</div>
					</el-form-item>
				</el-col>
...
onMounted(() => {
	...
	resizeObserver = new ResizeObserver(setColSpan);
	if (container.value) {
